...
namespace AoC2020.Providers
{
    ...
    public class DayTwoSolver : ISolver<int>
    {
        ...
        public int StepA()
        {
            var l = List.Select(c => c.Trim().Split(' '));
            var result = l.Select(a => new PWPartOne
            {
                Min = Convert.ToInt32(a[0].Split('-')[0]),
                Max = Convert.ToInt32(a[0].Split('-')[1]),
                Letter = a[1].Substring(0, 1),
                Password = a[2],
            }).ToArray();
            var valid = result.Where(c => c.PasswordLetterCount >= c.Min && c.PasswordLetterCount <= c.Max);
        ...
        {
            var l = List.Select(c => c.Trim().Split(' '));
            var result = l.Select(a => new PWPartTwo
            {
                PosOne = Convert.ToInt32(a[0].Split('-')[0]),
                PosTwo = Convert.ToInt32(a[0].Split('-')[1]),
                Letter = a[1].Substring(0, 1),
                Password = a[2],
            });

            var valid = result.Where(c => ContainsOnlyPosition(c, true) || ContainsOnlyPosition(c, false));
            return valid.Count();
        }

        private bool ContainsOnlyPosition(PWPartTwo c, bool mustContainFirstPos)
        {
            var posOneZeroBased = c.PosOne - 1;
            var posTwoZeroBased = c.PosTwo - 1;
            if (mustContainFirstPos)
            {
                return c.Password[posOneZeroBased].ToString() == c.Letter
                    && c.Password[posTwoZeroBased].ToString() != c.Letter;
            }
            else
            {
                return c.Password[posTwoZeroBased].ToString() == c.Letter
                    && c.Password[posOneZeroBased].ToString() != c.Letter;
            }
        }

        public class PWPartOne
        {
            ...
            public string Password
            {
                ...
                {
                    _Password = value;
                    PasswordLetterCount = value.Count(f => f.ToString() == Letter);
                }
            }

            public int PasswordLetterCount { get; set; }
        }

        public class PWPartTwo
        {
            public int PosOne { get; set; }
            public int PosTwo { get; set; }
            public string Letter { get; set; }
            public string Password { get; set; }
        }

        private static IList<string> List = new[]
        {
            "4-5 t: ftttttrvts                  ",
            "7-8 k: kkkkkkkf                    ",
            "4-6 k: gqjkkk                      ",
